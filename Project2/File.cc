...
void File :: GetPage (Page *putItHere, off_t whichPage) {
	...
	ssize_t t = read (myFilDes, bits, PAGE_SIZE);
...
void File :: AddPage (Page *addMe, off_t whichPage) {
	...
	if (whichPage >= curLength) {		
		...
		for (off_t i = curLength; i < whichPage; i++) {
			...
			ssize_t t = write (myFilDes, &foo, sizeof (int));
		}
	...
	addMe->ToBinary (bits);
	lseek (myFilDes, PAGE_SIZE * whichPage, SEEK_SET);
	ssize_t t1 = write (myFilDes, bits, PAGE_SIZE);
	delete [] bits;
#ifdef F_DEBUG
...
void File :: Open (int fileLen, const char *fName) {
	...
	if (fileLen != 0) {
		...
		ssize_t t = read (myFilDes, &curLength, sizeof (off_t));
...
int File :: Close () {
	...
	lseek (myFilDes, 0, SEEK_SET);
	ssize_t t = write (myFilDes, &curLength, sizeof (off_t));
