...
Schema :: Schema (const char *fName, const char *relName) {
	...
	numAtts = 0;
	while (1) {
		int fc3 = fscanf (foo, "%s", space);
		if (!strcmp (space, "END")) {
			break;		
		} else {
			int fc4 = fscanf (foo, "%s", space);
			numAtts++;
		}
	}
	...
	for (int i = 0; i < numAtts; i++ ) {
		...
		int fc6 = fscanf (foo, "%s", space);	
		myAtts[i].name = strdup (space);

		// read in the attribute type
		int fc7 = fscanf (foo, "%s", space);
		if (!strcmp (space, "Int")) {
			myAtts[i].myType = Int;
